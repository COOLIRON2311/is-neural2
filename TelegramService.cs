...
namespace AIMLTGBot
{
    public class TelegramService : IDisposable
    {
        ...
        {
            aiml = aimlService;
            net1 = new AccordNet(new int[]{ 56, 300, 10});
            net2 = new StudentNetwork(new int[] { 56, 28, 14, 10 });
        ...
        async Task HandleUpdateMessageAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            if (message.Type == MessageType.Text)
            {
                ...
                if (messageText == "train")
                {
                    ...
                    net1.TrainOnDataSet(s, 20, 0.0005, true);
                    Console.WriteLine("Accurancy: " + net1.GetAccuracy(s));
                    net2.TrainOnDataSet(s, 200, 0.0005, true);
